...
class AlibabaCloudClient:
    ...
    def list_accounts(self):
        def _build_request(page_number: int) -> CommonRequest:
            request = CommonRequest()
            request.set_accept_format("json")
            request.set_domain("resourcemanager.aliyuncs.com")
            request.set_method("POST")
            request.set_protocol_type("https")
            request.set_version("2020-03-31")
            request.set_action_name("ListAccounts")
    ...
    def get_kubernetes_clusters(self) -> Dict[str, Any]:
        ...
        request.set_protocol_type("https")
        request.set_domain(f"cs.{self.region_id}.aliyuncs.com")
        request.set_version("2015-12-15")

        request.add_header("Content-Type", "application/json")
        request.set_uri_pattern("/api/v1/clusters")

        try:
            return self.make_request(request)
        except Exception as e:
            raise APIError(f"Failed to fetch Kubernetes clusters: {str(e)}") from e
